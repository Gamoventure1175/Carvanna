...
const ServiceLogFormContext = createContext<ServiceLogFormType | undefined>(
  undefined,
);

export function ServiceLogFormContextProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  return (
    <ServiceLogFormContext.Provider
      value={{ isServiceLogFormOpen, setIsServiceLogFormOpen }}
    >
      {children}
    </ServiceLogFormContext.Provider>
  );
...
export const useServiceLogForm = () => {
  ...
  if (!context) {
    throw new Error(
      "useServiceLogForm must be used within a ServiceLogFormProvider",
    );
  }
  return context;
};
